...
import { PRODUCTS, FARM_ICONS } from "@/data/products";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";

// Build farm data
const farmMap: Record<string, { origin: string; products: typeof PRODUCTS; isOrganic: boolean }> = {};
PRODUCTS.forEach((p) => {
  if (!farmMap[p.farm]) farmMap[p.farm] = { origin: p.origin, products: [], isOrganic: false };
      ...
      <main className="pt-24 bg-cream min-h-screen">
        ...
        <div className="relative h-72 overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <p className="text-xs tracking-[0.2em] uppercase text-green-pale mb-3">Our Network</p>
            <h1 className="font-playfair font-black text-white text-5xl">Partner Farms</h1>
        ...
        <div className="bg-green py-8 px-20 grid grid-cols-4 divide-x divide-white/20">
          {[
            ...
            { num: `${PRODUCTS.filter(p=>p.isOrganic).length}`, label: "Organic Products" },
            { num: "60mi", label: "Max Distance" },
            { num: "24h", label: "Harvest to Door" },
          ].map((s) => (
            <div key={s.label} className="text-center px-8">
              <div className="font-playfair text-3xl font-bold text-white">{s.num}</div>
          ...
          {farms.map(([name, info]) => (
            ...
            >
              ...
              <div className="bg-green-pale px-6 pt-8 pb-6">
                ...
                <h2 className="font-playfair font-bold text-verdant-dark text-xl">{name}</h2>
              ...
              <div className="px-6 py-5">
                <p className="text-xs uppercase tracking-wider text-[#aaa] mb-3">
                  {info.products.length} product{info.products.length !== 1 ? "s" : ""}
                </p>
                <div className="flex flex-col gap-2">
                  ...
                  {info.products.length > 4 && (
                    <Link href={`/shop?farm=${encodeURIComponent(name)}`} className="text-xs text-green hover:opacity-70 transition-opacity mt-1">
